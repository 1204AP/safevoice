...
//     body: 'Controlling your access to money, preventing you from working, or making you ask for every penny are red flags of abuse.',
//   },
//   {
//     icon: '📸',
//     title: 'Evidence Privacy',
//     body: 'Always save sensitive screenshots to the SafeVoice Vault rather than your phone\'s photo gallery.',
//   },
//   {
//     icon: '📞',
//     title: 'Local Support Lines',
//     body: 'If you need immediate shelter or a human to talk to, we have vetted local organisations available.',
//   },
//   {
//     icon: '⚡',
//     title: 'Quick Tip',
//     body: 'If you\'re in immediate danger, please call emergency services. This chat is for legal information and guidance only.',
//   },
// ];

// function parseLegalMessage(content) {
//   // Try to parse structured response
//   const hasSummary = content.includes('LEGAL SUMMARY') || content.includes('Legal Summary');
//   const hasSafetySteps = content.includes('IMMEDIATE SAFETY STEPS') || content.includes('Immediate Safety');
//   const hasNext = content.includes('NEXT') || content.includes('Next Recommended');

//   if (!hasSummary && !hasSafetySteps) return null;
...
//     if (/LEGAL SUMMARY|Legal Summary/i.test(line)) {
//       if (current.lines.length) sections.push(current);
//       current = { type: 'legal', lines: [] };
//     } else if (/IMMEDIATE SAFETY STEPS|Immediate Safety/i.test(line)) {
//       if (current.lines.length) sections.push(current);
//       current = { type: 'steps', lines: [] };
//     } else if (/NEXT RECOMMENDED|What to do NEXT/i.test(line)) {
//       if (current.lines.length) sections.push(current);
...
//         if (sec.type === 'text') {
//           return (
//             <p key={i} style={{ fontSize: '14px', color: '#334155', lineHeight: '1.6', marginBottom: '10px' }}>
//               {sec.lines.join(' ').trim()}
//             </p>
//           );
//         }
...
//               <div style={{ fontSize: '11px', fontWeight: '700', color: '#0369a1', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
//                 📋 Legal Summary
//               </div>
...
//               <div style={{ fontSize: '11px', fontWeight: '700', color: '#15803d', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
//                 ✅ Immediate Safety Steps
//               </div>
//               {steps.map((step, j) => (
//                 <div key={j} style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
//                   <span style={{ color: '#16a34a', fontWeight: '700', minWidth: '16px', fontSize: '13px' }}>{j + 1}.</span>
...
//               <span style={{ fontSize: '12px', color: '#0ea5e9', fontWeight: '500', cursor: 'pointer' }}>
//                 {sec.lines.join(' ').trim() || 'View Local Shelter Map →'}
//               </span>
//             </div>
//           );
//         }
...
//     {
//       role: 'assistant',
//       content: "I'm worried about my safety. My partner took my phone and I'm not allowed to leave the house without him. What can I do legally?",
//       isExample: true,
//     },
//     {
//       role: 'assistant',
//       content: `I'm here to help you understand your situation. What you've described—restricting your movement and controlling your communication—is often recognized by law as forms of domestic abuse and coercive control.

// LEGAL SUMMARY
// In many jurisdictions, preventing someone from leaving their home or controlling their access to communication is a criminal offense under 'Unlawful Imprisonment' or 'Coercive Control' statutes. You have the right to seek a Protection Order even if no physical violence has occurred yet.

// IMMEDIATE SAFETY STEPS
// 1. Create a 'Safe Word' with a trusted neighbor or friend that means 'Call for help'.
// 2. Keep a small 'Go Bag' hidden with essentials (ID, money, medication) if you find a window to leave.
// 3. Use 'Private Browsing' or 'Incognito Mode' for any further searches to stay discreet.`,
//     },
//   ]);
...
//     const newMessages = [...messages.filter(m => !m.isExample), userMsg];
...
//       setMessages((prev) => [...prev.filter(m => !m.isExample), { role: 'assistant', content: res.data.reply }]);
//     } catch {
//       setMessages((prev) => [
//         ...prev.filter(m => !m.isExample),
//         { role: 'assistant', content: 'Something went wrong. Please try again. If urgent, call 100.' },
//       ]);
//     }
//     setLoading(false);
//   };
...
//           <div style={{ background: '#eff6ff', borderRadius: '8px', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
...
//           </div>
//           <span style={{ fontSize: '18px', color: '#0f172a', fontWeight: '700', marginLeft: '8px' }}>
//             Secure Session Active
//           </span>
//           <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
//             <button
//               style={{ background: '#0ea5e9', border: 'none', borderRadius: '8px', padding: '7px 14px', fontSize: '12px', color: '#fff', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
//               onClick={() => saveToVault(messages.filter(m => !m.isExample).map(m => `[${m.role}] ${m.content}`).join('\n\n'))}
//             >
//               💾 SAVE CONVERSATION
//             </button>
//           </div>
...
//         <div style={{ background: '#fff', padding: '10px 20px', borderBottom: '1px solid #e2e8f0', textAlign: 'center' }}>
//           <p style={{ fontSize: '12px', color: '#64748b', margin: 0 }}>
//             Your messages are processed locally and not stored on our servers. Remember to clear your browser history or use 'Exit Now' if interrupted.
//           </p>
//         </div>
...
//                 </div>
//               ) : (
//                 <div style={{ maxWidth: '80%', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '18px 18px 18px 4px', padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
...
//                     <button onClick={() => saveToVault(msg.content)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}>
//                       💾 SAVE TO VAULT
//                     </button>
//                     <button style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}>
//                       📋 COPY STEPS
//                     </button>
//                     <button onClick={() => speak(msg.content)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}>
//                       🔊 LISTEN
//                     </button>
//                     <button style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}>
//                       📍 LOCAL SUPPORT
...
//         {/* Quick questions */}
//         <div style={{ padding: '0 16px 8px', display: 'flex', gap: '8px', overflowX: 'auto', background: '#fff', borderTop: '1px solid #f1f5f9' }}>
//           {QUICK_QUESTIONS.map((q, i) => (
//             <button
//               key={i}
//               onClick={() => send(q)}
//               style={{ whiteSpace: 'nowrap', padding: '7px 12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '20px', fontSize: '12px', color: '#475569', cursor: 'pointer', marginTop: '8px' }}
//             >
//               {q}
//             </button>
//           ))}
...
//           <button
//             onClick={startVoice}
//             style={{ background: listening ? '#ef4444' : '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '50%', width: '44px', height: '44px', cursor: 'pointer', fontSize: '18px' }}
//           >
//             🎤
//           </button>
//           <button
...
//       {/* Sidebar */}
//       <div style={{ width: '220px', background: '#fff', borderLeft: '1px solid #e2e8f0', overflowY: 'auto', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
...
//               <span style={{ fontSize: '12px', fontWeight: '600', color: '#0f172a' }}>{tip.title}</span>
//             </div>
